...
class LoABot:
    ...
    def _initialize_boss_data(self):
        ...
        for aciklama in valid_boss_keys:
            ...
            # YAML'da koordinat_ref doğrudan {x, y} dict'i olabilir (yeni format)
            # ya da self.coordinates'e atıfta bulunan string olabilir (eski format)
            if isinstance(coord_ref, dict) and "x" in coord_ref and "y" in coord_ref:
                ...
                self.log(f"Config hatası: Boss '{aciklama}' için 'koordinat_ref' geçersiz format, hedef atlanıyor.")
                del self.bosslar[aciklama]
                continue
    ...
    def _preload_images_from_config(self):
        ...
        try:
            ...
            # 5. Demon Boss 'bosslar' (YAML alanları: anchor, area_check, spawn_check, victory)
            for boss_data in self.demon_boss_cfg.get("bosslar", {}).values():
                for field_name in ["anchor", "area_check", "spawn_check", "victory"]:
                    field_data = boss_data.get(field_name)
                    if field_data and isinstance(field_data, dict) and field_data.get("image_file"):
                        image_files_to_load.add(field_data["image_file"])
        ...
        for step in sequence:
            ...
            try:
                if action == "click":
                    ...
                    # YAML yeni format: koordinat_ref doğrudan {x, y} dict'i
                    if isinstance(koordinat_ref, dict) and "x" in koordinat_ref and "y" in koordinat_ref:
                        ...
                        # Eski format: string etiket → coord_map'te ara
                        boss_coord_label = koordinat_ref
                        coord = coord_map.get(boss_coord_label)
                        if not coord:
                            self.log(f"Sekans Hatası: Boss '{boss_aciklama}' için boss koordinatı ('{boss_coord_label}') bulunamadı.")
                            return False
                    else:
                        self.log(f"Sekans Hatası: Boss '{boss_aciklama}' için 'koordinat_ref' geçersiz veya eksik.")
                        return False

                    # Görsel doğrulama: YAML'da 'anchor' alanı kullanılıyor (yeni), 'gorsel_dogrulama' eski format
                    boss_verify_config = context_target.get("anchor") or context_target.get("gorsel_dogrulama")
                    if boss_verify_config and isinstance(boss_verify_config, dict):
                        ...
                        if image_file and region_key:
                            ...
                            if search_region:
                                log_msg = f"Doğrulama (Boss Anchor): '{image_file}' aranıyor..."
                                self.log(log_msg)
                                ...
                                if not match:
                                    log_msg = f"UYARI: Boss '{image_file}' bulunamadı. Yine de tıklanıyor..."
                                    self.log(log_msg)
                                    self.gui_queue.put(("set_status_line1", log_msg))
                                else:
                                ...
                                self.log(f"Sekans Hatası: Boss '{boss_aciklama}' için '{region_key}' bölgesi bulunamadı. Doğrulama atlandı.")
                        else:
                            self.log(f"Sekans UYARISI: Boss '{boss_aciklama}' için doğrulama görseli tanımı eksik. Atlandı.")
    ...
    def _attack_boss(self, hedef: dict) -> bool:
        ...
        if self.location_manager.get_region_name().upper() != target_location_name:
            ...
            # Alan kontrolü: YAML boss.area_check ile doğru alanda olduğumuzu teyit et
            ...
            if area_check_cfg and isinstance(area_check_cfg, dict):
                ...
                if ac_image and ac_region:
                    ...
                    if not ac_match:
                        self.log(f"UYARI: Alan kontrolü başarısız ('{ac_image}'). Yine de devam ediliyor.")
                    else:
        ...
        # --- SPAWN CONFIRM: YAML'daki spawn_check alanı ile boss doğuşunu teyit et ---
        spawn_confirm_enabled = self.settings.get("SPAWN_CONFIRM_ENABLED", False)
        spawn_check_cfg = hedef.get("spawn_check")
        if spawn_confirm_enabled and spawn_check_cfg and isinstance(spawn_check_cfg, dict):
            sc_image = spawn_check_cfg.get("image_file")
            sc_region_key = spawn_check_cfg.get("region_key", "region_full_screen")
            sc_confidence = float(spawn_check_cfg.get("confidence", 0.5))
            sc_region = self.ui_regions.get(sc_region_key)
            pre_window = self.settings.get("SPAWN_CONFIRM_PRE_WINDOW_SN", 4)
            timeout = self.settings.get("SPAWN_CONFIRM_TIMEOUT_SN", 3)
            poll = self.settings.get("SPAWN_CONFIRM_POLL_SN", 0.25)
            spawn_required = self.settings.get("SPAWN_CONFIRM_REQUIRED", False)

            if sc_image and sc_region:
                self.log(f"Spawn teyidi bekleniyor ({sc_image}, {timeout}sn pencere)...")
                deadline = time.time() + pre_window + timeout
                confirmed = False
                while time.time() < deadline:
                    try:
                        match = self.find_image_on_screen(sc_image, sc_region, sc_confidence)
                        if match:
                            self.log(f"Spawn teyit edildi: '{sc_image}' ekranda görüldü.")
                            confirmed = True
                            break
                    except Exception:
                        pass
                ...
                if not confirmed:
                    if spawn_required:
                        self.log(f"SPAWN TEYIT BAŞARISIZ ({hedef['aciklama']}). 'spawn_confirm_required: true' → Saldırı iptal.")
                        return False
                    else:
                        self.log(f"Spawn teyit edilemedi ({hedef['aciklama']}), yine de devam ediliyor.")
        # --- SPAWN CONFIRM SONU ---
    ...
    def verify_boss_kill(self, boss) -> bool:
        # YAML'da her boss için 'victory' alanı: {image_file, region_key, confidence}
        victory_cfg = boss.get("victory")
        if victory_cfg and isinstance(victory_cfg, dict):
            image_file = victory_cfg.get("image_file")
            region_key = victory_cfg.get("region_key", "region_full_screen")
            confidence = float(victory_cfg.get("confidence", 0.4))
            region = self.ui_regions.get(region_key) or self.ui_regions.get("region_full_screen")
            if image_file and region:
                try:
                    match = self.find_image_on_screen(image_file, region, confidence)
                    if match:
                        self.log(f"{boss['aciklama']} için zafer ikonu görüldü ({image_file}).")
                        return True
                except Exception:
                    pass
        else:
            # Fallback: genel victory_icon.png
            try:
