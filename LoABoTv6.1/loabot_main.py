...
from environment_manager import LocationManager, PopupManager
...
class LoABot:
    def __init__(self, root):
        ...
        # v6.1 core entegrasyonu: GameState + EventBus + BotBridge
        try:
            ...
            self.event_bus = None
            self.bridge = None
            self.log("v6.1 core bulunamadi, v5.9 modunda devam ediliyor.", level="WARNING")
        ...
        self.log("LoABot v6.1: stabil runtime + GameState/EventBus/Bridge aktif")
    ...
    def auto_start_recording(self, trigger_type: str = "manual", timeout_sec: float = 0) -> bool:
        """
        v6.1: Video kaydi yalnizca kullanici istegiyle baslatilir.
        GUI butonundan veya harici komutla tetiklenir.
        """
        if getattr(self, "_manual_recording", False):
            return False
        if getattr(self, "_auto_log_active", False):
        ...
        self.log(f"[REC] Kayit baslatiliyor. Tetik: {trigger_type}", level="DEBUG")
        ...
        # Opsiyonel guvenlik zamanlayicisi
        if timeout_sec > 0:
    ...
    def auto_stop_recording(self):
        """v6.1: Kaydi guvenli sekilde durdurur."""
        if not getattr(self, "_auto_log_active", False):
            return

        trigger = str(getattr(self, "_auto_log_trigger", "")).strip().lower()
        self.log("[REC] Kayit durduruluyor.", level="DEBUG")
        self._auto_log_active = False
        ...
        vid = getattr(self, "video_recorder", None)
        if vid is not None and vid.is_recording:
            vid.stop(success=True, reason=f"stop_{trigger}")
