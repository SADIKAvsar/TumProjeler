...
class PopupManager:
    ...
    def monitor_thread(self):
        ...
        if not self.popups:
            self.bot.log("⚠️ PopupManager: Kapatılacak pop-up listesi boş, izleme pasif.")
            return
        ...
        interval = self.cfg.get("check_interval_sn", 20) # Kontrol sıklığı

        while True:
            # Bot duraklatılmışsa veya başka bir kritik aksiyon (savaş gibi) varsa bekle
            ...
            for p in self.popups:
                ...
                # VisionManager üzerinden resmi tara (1440p Gigabyte uyumlu)
                match = self.bot.vision.find(image_name, reg, confidence)
                
                if match:
                    image_l = str(image_name or "").lower()
                    is_dc_popup = bool(p.get("dc_popup", False)) or any(
                        token in image_l for token in ("dc", "disconnect", "network", "reconnect", "error", "hata")
                    )
                    try:
                        self.bot.vision.save_system_shadow_frame(
                            stage="popup_detected",
                            namespace="system_recovery",
                            reason=image_name,
                        )
                        if is_dc_popup:
                            self.bot.vision.save_system_shadow_frame(
                                stage="dc_popup_detected",
                                namespace="system_recovery",
                                reason=image_name,
                            )
                    except Exception:
                    ...
                    # Automator'ın click yeteneğini kullan
                    self.bot.automator.click(f"popup_{image_name}", {"x": match[0], "y": match[1]})
                    time.sleep(1) # Ardışık pop-up'lar için kısa bekleme
